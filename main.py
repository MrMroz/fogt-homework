...
charge = pc.OscillatingCharge(
    origin=(0, 0, 0), direction=(1, 0, 0), amplitude=2e-9, omega=7.49e16
)
...
Q = ax.quiver(
    x_quiver, z_quiver, x_quiver[:, 0, :], z_quiver[:, 0, :], scale_units="xy"
)
...
n_frames = 18  # Number of frames in gif
dt = 2 * np.pi / charge.omega / n_frames
ani = FuncAnimation(
    fig, _update_animation, frames=n_frames, blit=False, init_func=_init_animate
)
plt.show()
